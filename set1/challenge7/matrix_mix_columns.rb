...
module Matrix
  class Matrix_128
    def mix_columns
      mix_column_matrix_128 = MixColumns.mix_columns_matrix_128

      new_rows = ROW_COUNT.times.map do |row_number|
        ...
        COLUMN_COUNT.times.map do |col_number|
          ...
          row.zip(column).reduce(0) do |result, bytes|
            mult = case bytes[0]
              when 1 then bytes[1]
              when 2 then MixColumns.mult_2 bytes[1] if bytes[0] == 2
              when 3 then MixColumns.mult_3 bytes[1] if bytes[0] == 3
